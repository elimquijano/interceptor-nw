...
def handle_client(client_socket, client_address, device_port, traccar_port):
    ...
    try:
        ...
        if traccar_socket:
            ...
            print(f"Proxy: Closed connection to Traccar on port {traccar_port}")
        if additional_socket:
            additional_socket.close()
            print(f"Proxy: Closed connection to additional port {ADDITIONAL_PORT}")
        client_socket.close()
